...
public class SortSelectedRowsCommand extends GridCommand {
  ...
  public void run(CsvGridSheetPane gridSheetPane) {
    ...
    List<SortCriteria> criterias = SortCommand.chooseSortCriteria(gridSheetPane);
    if (criterias != null) {
      DefaultGridSheetSelectionModel sm = gridSheetPane.getSelectionModel();
      if (sm.getMainMinRowSelectionIndex() <= 0
          && gridSheetPane.getRowCount() - 1 <= sm.getMainMaxRowSelectionIndex()) {
        // entire rows are selected
        ((CsvGridSheetModel) gridSheetPane.getModel()).sort(criterias);
      } else {
        ((CsvGridSheetModel) gridSheetPane.getModel()).sort(criterias, sm.getSelectedRows());
      }
    }
  }
}
